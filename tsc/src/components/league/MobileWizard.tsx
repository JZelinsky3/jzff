...
function StepMembers({
  ...
  return (
    <StepShell
      title="Review the"
      titleEm="roster."
      subtitle="Merge cross-platform names, mark alumni, hide test accounts. All optional."
      cta={
        <>
    ...
    >
      <div className="mwiz-stat-row">
        <div className="mwiz-stat">
          <span className="mwiz-stat-val">{visible.length}</span>
          <span className="mwiz-stat-label">Managers</span>
        </div>
        <div className="mwiz-stat">
          <span className="mwiz-stat-val">{merged}</span>
          <span className="mwiz-stat-label">Merged</span>
        </div>
        <div className="mwiz-stat">
          <span className="mwiz-stat-val">{profiles.filter((p) => p.is_alumni_override).length}</span>
          <span className="mwiz-stat-label">Alumni</span>
        </div>
      </div>

      <div className="mwiz-list">
        {visible.slice(0, 8).map((p) => (
          <div key={p.id} className="mwiz-source-pill">
            <div className="mwiz-source-platform">{p.is_alumni_override ? 'ALUMNI' : 'ACTIVE'}</div>
            <div className="mwiz-source-id">{p.canonical_name}</div>
            {p.managers.length > 1 && <span className="mwiz-source-check" aria-label="Merged">∞</span>}
          </div>
        ))}
        {visible.length > 8 && (
          <div className="mwiz-list-more">+ {visible.length - 8} more on the full editor</div>
        )}
      </div>
    </StepShell>
  )
