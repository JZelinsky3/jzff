...
export type Question = {
  ...
  source: string
  /** Four names, the right one among them, in a DELIBERATELY SHUFFLED order.
   *  Every question was first written with the answer at the top, which made
   *  the whole exam "tap the first option" for twenty out of twenty. The
   *  answer now sits at each of the four positions five times, with no three
   *  questions in a row sharing a slot. Four, not twelve: a one-in-twelve
