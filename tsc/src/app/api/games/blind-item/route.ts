// GET /api/games/blind-item?pool=demo|<league-slug>&seed=ABCD1234
//
// Deals one card of Blind Item: eight redacted manager-seasons, plus the
...
//
// The page SSRs its opening card through the same dealBlindItem() this calls;
...
export async function GET(req: NextRequest) {
  ...
  const result = await dealBlindItem(params.get('pool') ?? '', params.get('seed'))
